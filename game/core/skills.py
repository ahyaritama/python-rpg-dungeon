...
from ..util import clear_screen, set_player_skills, set_player_stats
...
def _learn_skill(player: Player, skill_node: SkillNode, skill_id: int):
    ...
    if skill_id == skill_node.data.id:
        if skill_node.data.name in player.skills:
        ...
        if (
            ...
            player.skills.add(skill_node.data.name)
            set_player_skills(player.name, player.skills)
    ...
    if not skill_node.data.name in player.skills:
...
def _show_locked_skill(player: Player, skill_tree: SkillTree) -> bool:
    ...
    for data in skill_list:
        if not data.name in player.skills:
